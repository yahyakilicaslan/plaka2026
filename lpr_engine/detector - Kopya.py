...
print("--- FINAL SYSTEM v58.0 (QUEUE FLUSH & UI FIX) ---")
...
class OCRWorker(threading.Thread):
    ...
    def process(self, camera, img, full_frame):
        try:
            ...
            if cleaned:
                # GÜVENLİK KİLİDİ: Son okunan plaka buysa ve üzerinden 10 saniye geçmediyse Yoksay.
                ...
                if len(camera.plate_buffer) >= 3:
                    ...
                    if matches >= 2:
                        print(f"🚀🚀🚀 [{camera.camera_id}] TESPİT EDİLDİ: {most_common}")
                        ...
                        # --- KRİTİK DÜZELTME: HAFIZAYI TEMİZLE (QUEUE FLUSH) ---
                        # Kapı açıldığı an, sırada bekleyen diğer kareleri sil ki eski plaka tekrar girmesin.
                        with ocr_queue.mutex:
                            ocr_queue.queue.clear()
                        
                        camera.plate_buffer = [] # Buffer'ı da sil
                        ...
                        # Kararsızlık varsa bufferı kısalt
...
class CameraStream(threading.Thread):
    def __init__(self, slot_id):
        super().__init__(); self.slot_id = slot_id; self.camera_id = f"CAM-0{slot_id}"; self.config = None; self.cap = None; self.is_connected = False; self.running = True; self.cap_lock = threading.Lock(); self.plate_buffer = []; self.last_sent_plate = ""; self.last_sent_time = 0; self.update_black_frame("BASLATILIYOR")
            ...
            with buffer_lock:
                ...
                if self.camera_id in detection_overlays:
                    ...
                    if overlay.get('text') and time.time() < overlay.get('timer', 0):
                        plate = overlay['text']
                        cv2.rectangle(display_frame, (50, 50), (300, 110), (0, 128, 0), -1); cv2.putText(display_frame, plate, (60, 95), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 255), 3)
...
if __name__ == "__main__":
    # İŞLEMCİ YÜKÜNÜ DAĞITMAK İÇİN ÇİFT WORKER
    OCRWorker().start()
    OCRWorker().start()
